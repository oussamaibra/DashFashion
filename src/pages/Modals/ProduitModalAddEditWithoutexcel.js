...
import {
  ...
  const ProduitModalAddEditWithoutexcel = (props) => {
    ...
    const serverURL = "https://www.rafrafi.shop:8443";
  
    const [form] = useForm();
  
    useEffect(() => {
      axios
        .get("https://www.rafrafi.shop:8443/categories")
        .then((response) => {
      ...
      axios
        .get("https://www.rafrafi.shop:8443/collection")
        .then((response) => {
          console.log("response", response);
          if (response.data.data) {
      ...
      try {
        ...
        info?.fileList?.forEach((el) => {
          if (!isNil(el?.originFileObj?.name)) {
            ...
            Listimages.push(
              "https://www.rafrafi.shop:8443" + "/images/" + el?.name
            );
            setimages(oldimges);
            ...
            listOfPromise.push(
              axios({
                method: "post",
                url: "https://www.rafrafi.shop:8443" + col,
                data: bodyFormData,
    ...
    const handleonfinish = async (val) => {
      ...
      if (props.type === "EDIT") {
        await axios
          .put("https://www.rafrafi.shop:8443/products/" + values.id, {
            name: values.name,
        ...
        await axios
          .post("https://www.rafrafi.shop:8443/products", {
            name: values.name,
            description: values.description,
            detail: values.detail,
            categoryId: values.categoryId,
  ...
  export default ProduitModalAddEditWithoutexcel;
