...
const CollectionModalAddEdit = (props) => {
      ...
      info?.fileList?.forEach((el) => {
        if (
          ...
          !listfilesuploaded?.find(
            (val) =>
              val ===
              "https://www.rafrafi.shop:8443" + "/images/" + el?.originFileObj?.name
          )
        ) {
          ...
          listOfPromise.push(
            axios({
              method: "post",
              url: "https://www.rafrafi.shop:8443" + "/api/upload",
              data: bodyFormData,
  ...
  const handleonfinish = async (val) => {
    ...
    if (props.type === "EDIT") {
      await axios
        .put("https://www.rafrafi.shop:8443/collection/" + values.id, {
          name: values?.name,
      ...
      await axios
        .post("https://www.rafrafi.shop:8443/collection", {
          name: values?.name,
          description: values?.description,
          thumbnailImage: img[0],
        })
